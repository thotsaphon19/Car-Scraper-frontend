...
// placeholder รูปรถตามยี่ห้อ
const BRAND_PLACEHOLDERS={
  toyota:'https://upload.wikimedia.org/wikipedia/commons/thumb/9/9d/Toyota_hilux_revo_2015.jpg/320px-Toyota_hilux_revo_2015.jpg',
  isuzu:'https://upload.wikimedia.org/wikipedia/commons/thumb/4/4e/2020_Isuzu_D-Max_1.9_L_B_Blue_Power_facelift.jpg/320px-2020_Isuzu_D-Max_1.9_L_B_Blue_Power_facelift.jpg',
  ford:'https://upload.wikimedia.org/wikipedia/commons/thumb/6/6d/2019_Ford_Ranger_XL_pickup_%28Australia%29%2C_front_8.16.19.jpg/320px-2019_Ford_Ranger_XL_pickup_%28Australia%29%2C_front_8.16.19.jpg',
};

function getPlaceholder(title=''){
  const t=title.toLowerCase();
  if(t.includes('toyota')||t.includes('hilux')||t.includes('fortuner'))return BRAND_PLACEHOLDERS.toyota;
  if(t.includes('isuzu')||t.includes('d-max')||t.includes('mu-x'))return BRAND_PLACEHOLDERS.isuzu;
  if(t.includes('ford')||t.includes('ranger')||t.includes('everest'))return BRAND_PLACEHOLDERS.ford;
  return null;
}
...
function CarCard({car}){
  ...
  const fallback=getPlaceholder(car.title);
  const imgSrc=(!car.imageUrl||imgErr)?fallback:car.imageUrl;
    ...
    <a href={car.listingUrl||'#'} target="_blank" rel="noopener noreferrer" style={{textDecoration:'none',color:'inherit',display:'block',height:'100%'}}>
      <div onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}
        ...
        <div style={{height:152,background:'#f8fafc',position:'relative',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
          {imgSrc
            ?<img src={imgSrc} alt={car.title} style={{width:'100%',height:'100%',objectFit:'cover'}} onError={()=>setImgErr(true)}/>
            :<div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:4}}>
              <i className="ti ti-car" style={{fontSize:40,color:'#cbd5e1'}}></i>
              <span style={{fontSize:10,color:'#cbd5e1'}}>ไม่มีรูป</span>
            </div>
          }
...
// ── ScrapeModal ───────────────────────────────────────────────────────────────
function ScrapeModal({open,onClose,onDone}){
  ...
  useEffect(()=>{
    if(!open)return;
    setPhase('running');setCurrent(0);setPcts({});setTotalPct(0);setLog([]);
    let src=0,acc=0;
    iRef.current=setInterval(()=>{
      if(src>=SOURCES_LIST.length){
        clearInterval(iRef.current);
        setTotalPct(100);setPhase('done');
        onDone();
        return;
      }
      ...
      setPcts(p=>{
        const cur=p[key]||0;
        if(cur>=100){acc+=SOURCES_LIST[src].pct;setTotalPct(Math.min(Math.round(acc),99));setCurrent(s=>s+1);src++;return p;}
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200}}>
      <div style={{background:'#fff',borderRadius:20,padding:32,width:480,maxWidth:'92vw',boxShadow:'0 24px 64px rgba(0,0,0,.18)'}}>
        <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:24}}>
          <div style={{width:44,height:44,borderRadius:12,background:`linear-gradient(135deg,${phase==='done'?'#16a34a,#22c55e':'#1d4ed8,#3b82f6'})`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
            <i className={`ti ti-${phase==='done'?'check':'refresh'}`} style={{fontSize:22,color:'#fff'}}></i>
          </div>
          <div>
            <div style={{fontSize:16,fontWeight:700,color:'#111'}}>{phase==='done'?'ดึงข้อมูลเสร็จแล้ว! 🎉':'กำลังดึงข้อมูลรถ...'}</div>
            <div style={{fontSize:12,color:'#6b7280',marginTop:2}}>{phase==='done'?'อัปเดตหน้าแสดงผลแล้ว':'รันบนโน้ตบุ๊ค: node run-scraper.js'}</div>
          </div>
        </div>
        ...
        <div style={{marginBottom:20}}>
          ...
          <div style={{height:12,background:'#e2e8f0',borderRadius:99,overflow:'hidden'}}>
            <div style={{height:'100%',width:`${totalPct}%`,background:'linear-gradient(90deg,#1d4ed8,#3b82f6)',borderRadius:99,transition:'width .3s ease'}}></div>
          </div>
        </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:20}}>
          {SOURCES_LIST.map((src,i)=>{
            ...
            const pct=done?100:(pcts[src.key]||0);
            const status=done?'done':active?'active':'wait';
            return(
              <div key={src.key} style={{padding:'10px 14px',borderRadius:12,
                background:status==='done'?'#f0fdf4':status==='active'?'#eff6ff':'#f9fafb',
                border:`1px solid ${status==='done'?'#bbf7d0':status==='active'?'#bfdbfe':'#f1f5f9'}`,
                transition:'all .3s'}}>
                <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:active?8:0}}>
                  ...
                  <span style={{fontSize:12,fontWeight:700,
                    color:status==='done'?'#16a34a':status==='active'?'#1d4ed8':'#9ca3af'}}>
                    {status==='done'?'✅ เสร็จ':status==='active'?`${pct}%`:'⏳ รอ'}
                  </span>
                </div>
                {active&&(
                  <div style={{height:4,background:'#dbeafe',borderRadius:99,overflow:'hidden'}}>
                    <div style={{height:'100%',width:`${pct}%`,background:'#3b82f6',borderRadius:99,transition:'width .15s'}}></div>
                  </div>
                )}
              </div>
            );
        ...
        {phase==='done'
          ...
          :<div style={{textAlign:'center',fontSize:12,color:'#94a3b8'}}>⏱ ประมาณ 10-15 นาที — หน้าจะอัปเดตอัตโนมัติ</div>
        }
      </div>
    </div>
  );
...
function ScrapeStatusBar({history}){
  ...
  const stats=latest.sourceStats||{};
  return(
    <div style={{background:'#fff',borderRadius:14,padding:'14px 18px',marginBottom:14,border:'1px solid #f1f5f9',boxShadow:'0 1px 4px rgba(0,0,0,.06)'}}>
      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
        ...
        <span style={{fontSize:13,fontWeight:600,color:'#111'}}>สถานะการดึงข้อมูลล่าสุด</span>
      ...
      <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
        {SOURCES_LIST.map(src=>{
          ...
          const status=!info?'no-data':info.error?'error':info.blocked||info.count===0?'blocked':'ok';
          const colors={ok:'#f0fdf4,#16a34a,#bbf7d0',blocked:'#fffbeb,#b45309,#fcd34d',error:'#fef2f2,#dc2626,#fecaca','no-data':'#f9fafb,#9ca3af,#e5e7eb'};
          const[bg,tc,bc]=colors[status].split(',');
          return(
            <div key={src.key} style={{padding:'6px 12px',borderRadius:20,background:bg,border:`1px solid ${bc}`,display:'flex',alignItems:'center',gap:6}}>
              ...
              <span style={{fontSize:11,fontWeight:700,color:tc}}>
                {status==='ok'?`✅ ${info.count} คัน`:status==='blocked'?'⚠️ ถูกบล็อก':status==='error'?'❌ Error':'—'}
              </span>
            </div>
          );
        })}
      </div>
      {Object.values(stats).some(s=>s?.error)&&(
        <div style={{marginTop:10,padding:'8px 12px',background:'#fef2f2',borderRadius:8,border:'1px solid #fecaca'}}>
          {Object.entries(stats).filter(([,s])=>s?.error).map(([k,s])=>(
            <div key={k} style={{fontSize:11,color:'#dc2626'}}>❌ {k}: {s.error}</div>
          ))}
        </div>
      )}
    </div>
...
export default function Home(){
  ...
  useEffect(()=>{
    if(!ready)return;
    setLoading(true);fetchAll();fetchHistory();
  ...
  const handleDone=useCallback(()=>{
    setShowScrape(false);
    setLoading(true);
    Promise.all([fetchAll(),fetchHistory()]).then(()=>
  ...
  return(
    <div style={{minHeight:'100vh',background:'#f1f5f9'}}>
      {showScrape&&<ScrapeModal open={showScrape} onClose={()=>setShowScrape(false)} onDone={handleDone}/>}
      ...
      <div style={{background:'#fff',borderBottom:'1px solid #e2e8f0',padding:'0 24px',height:56,display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:10,boxShadow:'0 1px 3px rgba(0,0,0,.05)'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          ...
          <button onClick={()=>setShowScrape(true)}
            style={{background:'linear-gradient(135deg,#1d4ed8,#3b82f6)',border:'none',borderRadius:9,padding:'7px 16px',cursor:'pointer',fontSize:13,color:'#fff',display:'flex',alignItems:'center',gap:6,fontWeight:600,boxShadow:'0 2px 8px rgba(29,78,216,.3)'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8,paddingLeft:8,borderLeft:'1px solid #e2e8f0'}}>
            ...
            <button onClick={()=>{logout();router.push('/login');}}
              ...
              onMouseLeave={e=>e.target.style.background='none'}>ออก</button>
          </div>
        </div>
      </div>
      ...
      <div style={{maxWidth:1280,margin:'0 auto',padding:'24px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14,marginBottom:20}}>
          <div style={{background:'#fff',borderRadius:14,padding:'16px 18px',boxShadow:'0 1px 4px rgba(0,0,0,.06)',border:'1px solid #f1f5f9',gridColumn:'1/3'}}>
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
              <div style={{width:32,height:32,borderRadius:9,background:'#3b82f6',display:'flex',alignItems:'center',justifyContent:'center'}}>
                <i className="ti ti-car" style={{fontSize:16,color:'#fff'}}></i>
              </div>
              <span style={{fontSize:12,color:'#6b7280',fontWeight:500}}>รถทั้งหมด</span>
            </div>
            <div style={{fontSize:24,fontWeight:700,color:'#111',marginBottom:4}}>{fmt(stats?.total)} คัน</div>
            <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
              {Object.entries(by).map(([s,n])=>(
                <span key={s} style={{fontSize:11,padding:'2px 8px',borderRadius:20,background:'#f1f5f9',color:'#374151'}}>
                  {SRC[s]?.icon||'🚗'} {n}
                </span>
              ))}
            </div>
          ...
          <div style={{background:'#fff',borderRadius:14,padding:'16px 18px',boxShadow:'0 1px 4px rgba(0,0,0,.06)',border:'1px solid #f1f5f9'}}>
            <div style={{fontSize:12,color:'#6b7280',marginBottom:4}}>ราคาต่ำสุด / สูงสุด</div>
            <div style={{fontSize:14,fontWeight:700,color:'#16a34a'}}>{fmt(stats?.priceRange?.min)} ฿</div>
            <div style={{fontSize:14,fontWeight:700,color:'#dc2626'}}>{fmt(stats?.priceRange?.max)} ฿</div>
          </div>
          <div style={{background:'#fff',borderRadius:14,padding:'16px 18px',boxShadow:'0 1px 4px rgba(0,0,0,.06)',border:'1px solid #f1f5f9'}}>
            <div style={{fontSize:12,color:'#6b7280',marginBottom:4}}>ราคาเฉลี่ย</div>
            <div style={{fontSize:20,fontWeight:700,color:'#f59e0b'}}>{fmt(stats?.priceRange?.avg)} ฿</div>
          </div>
        </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 1fr 1fr',gap:10,marginBottom:10}}>
            <div style={{position:'relative'}}>
              ...
              <input value={kw} onChange={e=>setKw(e.target.value)} placeholder="ค้นหาชื่อรถ..."
                style={{...inp,paddingLeft:34}} onFocus={e=>e.target.style.borderColor='#3b82f6'} onBlur={e=>e.target.style.borderColor='#e5e7eb'}/>
            </div>
            <select value={brand} onChange={e=>{setBrand(e.target.value);setModel('');}} style={inp}
            ...
            <select value={model} onChange={e=>setModel(e.target.value)} disabled={!brand}
              style={{...inp,opacity:brand?1:.5}} onFocus={e=>e.target.style.borderColor='#3b82f6'} onBlur={e=>e.target.style.borderColor='#e5e7eb'}>
            ...
            <select value={selDate} onChange={e=>setSelDate(e.target.value)} style={inp}
              ...
              {history.map(h=>(
                <option key={h.date} value={h.date}>
                  {fmtDate(h.syncedAt)} — {h.total} คัน
                </option>
              ))}
            </select>
            <button onClick={()=>{setKw('');setBrand('');setModel('');setMn('');setMx('');setLoc('');setSrc('');setSelDate('');}}
              style={{padding:'9px 16px',borderRadius:10,border:'1.5px solid #e5e7eb',background:'#f8fafc',cursor:'pointer',fontSize:13,color:'#6b7280',display:'flex',alignItems:'center',gap:6,whiteSpace:'nowrap',fontFamily:'inherit'}}>
        ...
        {loading?(
          ...
          <div style={{textAlign:'center',padding:'80px 0',color:'#94a3b8'}}>
            ...
            <button onClick={()=>setShowScrape(true)}
              style={{background:'linear-gradient(135deg,#1d4ed8,#3b82f6)',color:'#fff',border:'none',borderRadius:10,padding:'10px 20px',cursor:'pointer',fontSize:13,fontWeight:600}}>
              ดึงข้อมูลใหม่
            </button>
          </div>
        ):(
