...
function CarCard({car}){
  ...
  const showImg=car.imageUrl&&!imgErr;
  return(
...
function ScrapeModal({onClose,onDone}){
  ...
  useEffect(()=>{
    ...
    ref.current=setInterval(()=>{
      if(src>=SOURCES_LIST.length){clearInterval(ref.current);setTotal(100);setDone(true);setTimeout(onDone,1500);return;}
      const key=SOURCES_LIST[src].key;
      setPcts(p=>{
        const cur=p[key]||0;
        if(cur>=100){acc+=SOURCES_LIST[src].pct;setTotal(Math.min(Math.round(acc),99));setCurrent(c=>c+1);src++;return p;}
      ...
      <div style={{background:'#fff',borderRadius:20,padding:32,width:480,maxWidth:'92vw',boxShadow:'0 24px 64px rgba(0,0,0,.18)'}}>
        ...
        <div style={{marginBottom:20}}>
          <div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}>
            ...
            <span style={{fontSize:14,fontWeight:700,color:'#1d4ed8'}}>{total}%</span>
          </div>
          <div style={{height:12,background:'#e2e8f0',borderRadius:99,overflow:'hidden'}}>
            <div style={{height:'100%',width:`${total}%`,background:'linear-gradient(90deg,#1d4ed8,#3b82f6)',borderRadius:99,transition:'width .3s'}}></div>
          </div>
        </div>
    ...
    <div style={{background:'#fff',borderRadius:14,padding:'14px 18px',marginBottom:14,border:'1px solid #f1f5f9',boxShadow:'0 1px 4px rgba(0,0,0,.06)'}}>
      ...
      <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
        {SOURCES_LIST.map(src=>{
          ...
          const[bg,tc,bc]=colors[st].split(',');
          return(
            <div key={src.key} style={{padding:'6px 12px',borderRadius:20,background:bg,border:`1px solid ${bc}`,display:'flex',alignItems:'center',gap:6}}>
              ...
              <span style={{fontSize:11,fontWeight:700,color:tc}}>
                {st==='ok'?`✅ ${info.count}`:st==='blocked'?'⚠️ บล็อก':st==='error'?'❌ Error':'—'}
              </span>
            </div>
          );
        })}
      </div>
      {Object.entries(stats).some(([,s])=>s?.error)&&(
...
export default function App(){
  ...
  useEffect(()=>{
    if(cars.length>0||!loading)return;
  },[src,kw,brand,model,mn,mx,loc,selDate]);

  const fetchAll=useCallback(async()=>{
    const p=new URLSearchParams();
    if(src)p.set('source',src);
    const kws=[kw,brand,model].filter(Boolean);
    if(kws.length)p.set('keyword',kws.join(' '));
    if(mn)p.set('minPrice',mn);if(mx)p.set('maxPrice',mx);
    if(loc)p.set('location',loc);if(selDate)p.set('date',selDate);
  ...
  useEffect(()=>{setLoading(true);fetchAll();},[src,kw,brand,model,mn,mx,loc,selDate]);

  const by=stats?.bySource||{};
  const sources=[{v:'',l:'ทั้งหมด',n:stats?.total||0},...Object.entries(SRC).map(([v,c])=>({v,l:c.label,n:by[v]||0})).filter(s=>s.n>0)];
    ...
    <div style={{minHeight:'100vh',background:'#f1f5f9'}}>
      ...
      <div style={{background:'#fff',borderBottom:'1px solid #e2e8f0',padding:'0 24px',height:56,display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:10,boxShadow:'0 1px 3px rgba(0,0,0,.05)'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          ...
          <button onClick={()=>{setLoading(true);fetchAll();}} style={{background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:9,padding:'6px 10px',cursor:'pointer',fontSize:12,color:'#374151'}}>
